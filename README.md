...
The dataset is super imbalanced — only 0.17% of transactions are actually fraud. So a model that just says "not fraud" every time already gets 99.83% accuracy, which is obviously useless. Had to use proper metrics like AUPRC instead of accuracy.
...
Download `creditcard.csv` from [Kaggle](https://www.kaggle.com/datasets/mlg-ulb/creditcardfraud), drop it in this folder, and run the notebook.
